...
def create_grid_layout(data):
    ...
    for pos, display_name in positions_col1:
        col1_layout.append([sg.Text(display_name, font=('Any', 15, 'bold'))])
...
for i in range(len(My_RBs)):
    RB_layout.append([sg.Text(f"{i+1}. "), sg.Text(My_RBs[i], key=f'RB_{i+1}')])
WR_layout = []
for i in range(len(My_WRs)):
    WR_layout.append([sg.Text(f"{i+1}. "), sg.Text(My_WRs[i], key=f'WR_{i+1}')])
QB_layout = []
for i in range(len(My_QBs)):
    QB_layout.append([sg.Text(f"{i+1}. "), sg.Text(My_QBs[i], key=f'QB_{i+1}')])
TE_layout = []
for i in range(len(My_TEs)):
    TE_layout.append([sg.Text(f"{i+1}. "), sg.Text(My_TEs[i], key=f'TE_{i+1}')])
