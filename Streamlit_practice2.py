...
def initial_page():
    st.header("Welcome to the Fantasy Football Draft Aid - 2023")
...
def main_page():
    st.markdown('<div class="main-title">Fantasy Football Draft Aid 2023</div>', unsafe_allow_html=True)
    st.write(' ')
                    ...
                    for key in data:
                        for idx, player_tuple in enumerate(data[key]):
                            if player_name == player_tuple[0]:
                                ...
                                st.session_state.data = data
                                # Append to recently_deleted_players
                                recently_deleted_players.append((removed_player, deleted_player[1], deleted_player[2], deleted_player[3], deleted_player[4]))
                                st.success(f"Successfully removed {removed_player} from the board.")  # Optional success message
                                st.rerun()
                                break     
